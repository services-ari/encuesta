...
function fh_title(){
	...
	switch (page) {
		...
		case 'plans': $title = 'Planes - '.site_title; break;
		case 'dashboard': $title = 'Dashboard - '.site_title; break;
		case 'rapport': $title = 'Reporte - '.site_title; break;
		case 'responses': $title = 'Respuestas - '.site_title; break;
		case 'newsurvey': $title = 'Nueva Encuesta - '.site_title; break;
		case 'userdetails': $title = 'Detalles - '.site_title; break;
...
function fl_user($id, $link = true, $cut = false, $count = 25){
	global $db;
	if(!$id){
		return false;
	}
  $sql = $db->query("SELECT id, oficio, plan FROM ".prefix."users WHERE id = '{$id}'");
  $rs = $sql->fetch_assoc();
	$color = ( $rs['plan']==1 ? '#00cec9' : ( $rs['plan']==2 ? '#ff7675' : ( $rs['plan']==3 ? '#6c5ce7' : '')));
	$oficio = $rs['oficio'];
	return ($link) ? '<a href="#"'.($color?' style="color:'.$color.'"':'').'>'.$oficio.'</a>' : $oficio;
}
...
function fh_ip(){
  foreach (array('HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR') as $key){
    if (array_key_exists($key, $_SERVER) === true){
        ...
        $ip = trim($ip); // Solo para estar seguros
