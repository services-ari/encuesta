...
<div class="row">
	<div class="col-6">
		...
		<div class="pt-div-stats">
			...
			<div><b><?=$lang['rapports']['roficio']?></b> <?=$rs['oficio']?></div>
		</div>
...
while($s_rs = $s_sql->fetch_assoc()){
	...
	while($q_rs = $q_sql->fetch_assoc()){
		...
		echo "<div class='pt-rapport-q'>
						...
						<h3>".$q_rs['title']."
							<div class='pt-options'>
							".(in_array($q_tp, ['stars', 'radio', 'checkbox'])? "<small class='showchart'>gráfico</small><small class='showpie'>Redondo</small>" :"<small class='showresults'>{$lang['rapports']['results']}</small><small class='exportEx'>{$lang['rapports']['export']}</small>")."
						</div></h3>";
