...
if($pg == 'rapport-stats') {
	...
	if(us_level == 6){
		if($request == "daily"){
		  ...
		  $aa['data'] = array_reverse($aa['data']);
		  $aa['labels'] = array_reverse($aa['labels']);
		  $aa['title'] = "Gráfico respuestas de ".$lang['dashboard']['stats_line_d'];
		} elseif($request == "monthly"){
			$aa = [];
			for ($i=1; $i <=12 ; $i++) {
		  ...
		  $aa['title'] = "Respuestas ".$lang['dashboard']['stats_line_m'];
		}
		echo json_encode($aa);
	}
	...
	if(us_level == 6){
		if($request == "daily"){
		  ...
		  $aa['data'] = array_reverse($aa['data']);
		  $aa['labels'] = array_reverse($aa['labels']);
		  $aa['title'] = "Gráfico encuestas de ".$lang['dashboard']['stats_line_d'];
		} elseif($request == "monthly"){
			$aa = [];
			for ($i=1; $i <=12 ; $i++) {
		  ...
		  $aa['title'] = "Encuestas ".$lang['dashboard']['stats_line_m'];
		}
		echo json_encode($aa);
	}
} elseif($pg == 'logout'){
		...
		if(empty($pg_subject) || empty($pg_email) || empty($pg_message)){
			...
			foreach ($emails as $em) {
				...
				if( $mail->send() ){
					$alert = [
						'type'  =>'success',
						'alert' => fh_alerts("Envíado con éxito.", 'success')
					];
				} else {
					$alert = [
						'type'  =>'danger',
