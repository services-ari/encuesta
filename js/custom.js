...
//- Cambio de tamaño automático de Textarea
...
$(".pt-sendsurveyemail").on("submit", function(){
	$.post(path+"/ajax.php?pg=sendsurveyemail", $(this).serialize(), function(puerto){
		if(puerto.type == 'success'){
			$.puerto_confirm("¡Muy Bien!", puerto.alert, "green");
		} else {
...
$("[name=bg_gradient]").on("change", function(){
	...
	var clr1 = ($("[name=bg_v1]").val()?$("[name=bg_v1]").val().replace(/<[^>]+>/g, ''):'#15224f');
	var clr2 = ($("[name=bg_v2]").val()?$("[name=bg_v2]").val().replace(/<[^>]+>/g, ''):'#3c4f91');
	if($("[name=bg_gradient]:checked").val() == 0){
...
trigger.livequery('submit', function(){
 var $this  = $(this).find('button[clicked=true]'),
		 ...
		 container.append('<div class="pt-survey-loading"><span><i class="fas fa-spinner fa-spin"></i> Cargando...</span></div>');
		 $(window).trigger('click');

		 if(step == "undefined" || survey == "undefined"){
				$(".pt-survey-loading span").prepend("<strong class='error'><i class='fas fa-times'></i> ¡Ocurre algo! <i>¡Inténtalo de nuevo!</i></strong>");
				setTimeout(function () {
...
$('.exportEx').on("click", function(e){
	...
	$.get(path+"/ajax.php?pg=rapport-stats&id="+id, function(puerto) {
		...
		if(as.type == 'error'){
			...
			console.log(puerto);
			$.puerto_confirm("Genial!", "El archivo está listo para descargar.", "green");
...
$("#pt-send-signup").on("submit", function(){
	...
	btn.prop('disabled', true).html('<i class="fas fa-spinner fa-pulse fa-fw"></i> Cargando..');
